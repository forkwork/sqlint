...
impl<'a> Mysql<'a> {
    ...
    fn visit_numeric_comparison(&mut self, left: Expression<'a>, right: Expression<'a>, sign: &str) -> visitor::Result {
        #[cfg(feature = "json")]
        fn json_to_sqlint_value<'a>(json: serde_json::Value) -> crate::Result<Value<'a>> {
            match json {
        ...
        match (left, right) {
            ...
            (left, right) if left.is_json_value() && right.is_fun_retuning_json() => {
                let sqlint_value = json_to_sqlint_value(left.into_json_value().unwrap())?;

                self.visit_parameterized(sqlint_value)?;
                self.write(format!(" {sign} "))?;
                self.visit_expression(right)?;
            }
            #[cfg(feature = "json")]
            (left, right) if left.is_fun_retuning_json() && right.is_json_value() => {
                let sqlint_value = json_to_sqlint_value(right.into_json_value().unwrap())?;
